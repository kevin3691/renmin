...
import com.zzb.zo.dao.MeetDao;
import com.zzb.zo.entity.Meet;
...
@Transactional
@Service("meetService")
public class MeetService extends BaseService<Meet> {
	@Resource
	...
	public QueryResult<Meet> list(HttpServletRequest request) {
		...
		String meetdNo = request.getParameter("meetdNo") != null ? request
				.getParameter("meetdNo") : "";
		...
		String hql = "FROM Meet WHERE 1=1";
		if (isActive > 0) {
			hql += " AND isActive=?";
			args.add(isActive);
		}
		...
		qp.setArgs(args);
		qp.setHql(hql);
		QueryResult<Meet> qr = meetDao.list(qp);
		return qr;
	}
	...
	public Meet save(Meet doc, HttpServletRequest request) {

		doc = meetDao.save(doc);


		return doc;
	}

	public Meet dtl(int id) {
		Meet doc = meetDao.dtl(id);
		return doc;
	}
	...
	public QueryResult<Meet> listByDocTypeId(int orgId) {
	...
	public QueryResult<Meet> listAllByDocTypeIds(List<Integer> Ids) {
		QueryPara qp = new QueryPara();
		Map<String, Object> alias = new HashMap<String, Object>();
		String hql = "FROM Meet WHERE 1=1";
		...
		qp.setAlias(alias);
		qp.setHql(hql);
		QueryResult<Meet> qr = meetDao.list4(qp);
		return qr;
	}



	public QueryResult<Meet> listAllUser() {
		QueryPara qp = new QueryPara();
		//List<Object> args = new ArrayList<Object>();
		String hql = "FROM Meet WHERE 1=1";
	...
	public QueryResult<Meet> listByOrgIds(List<Integer> idList) {
		QueryPara qp = new QueryPara();
		Map<String, Object> alias = new HashMap<String, Object>();
		String hql = "FROM Meet WHERE 1=1 ";
		hql += " AND docTypeId in (:idList)";
		...
		qp.setHql(hql);
		qp.setAlias(alias);
		QueryResult<Meet> qr = meetDao.listNoTotal(qp);
		return qr;
	}
