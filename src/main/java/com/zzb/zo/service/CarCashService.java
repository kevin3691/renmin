...
import com.zzb.zo.dao.CarCashDao;
import com.zzb.zo.entity.CarCash;
 ...
 */
@Transactional
@Service("carCashService")
public class CarCashService extends BaseService<CarCash> {
	@Resource
	private CarCashDao carCashDao;
	 ...
	 * @return
	 */
	public QueryResult<CarCash> list(HttpServletRequest request) {
		...
		String carCashdNo = request.getParameter("carCashdNo") != null ? request
				.getParameter("carCashdNo") : "";
		...
		String hql = "FROM CarCash WHERE 1=1";
		if (isActive > 0) {
			hql += " AND isActive=?";
			args.add(isActive);
		}
		...
		if (!carCashdNo.equals("")) {
			hql += " AND carCashdNo LIKE ?";
			args.add('%' + carCashdNo + '%');
		}
		...
		qp.setArgs(args);
		qp.setHql(hql);
		QueryResult<CarCash> qr = carCashDao.list(qp);
		return qr;
	}
	


	public CarCash save(CarCash doc, HttpServletRequest request) {

		doc = carCashDao.save(doc);


		return doc;
	}

	public CarCash dtl(int id) {
		CarCash doc = carCashDao.dtl(id);
		return doc;
	}
	...
	public QueryResult<CarCash> listByDocTypeId(int orgId) {
	...
	public QueryResult<CarCash> listAllByDocTypeIds(List<Integer> Ids) {
		QueryPara qp = new QueryPara();
		Map<String, Object> alias = new HashMap<String, Object>();
		String hql = "FROM CarCash WHERE 1=1";
		...
		qp.setAlias(alias);
		qp.setHql(hql);
		QueryResult<CarCash> qr = carCashDao.list4(qp);
		return qr;
	}



	public QueryResult<CarCash> listAllUser() {
		QueryPara qp = new QueryPara();
		//List<Object> args = new ArrayList<Object>();
		String hql = "FROM CarCash WHERE 1=1";
	...
	public QueryResult<CarCash> listByOrgIds(List<Integer> idList) {
		QueryPara qp = new QueryPara();
		Map<String, Object> alias = new HashMap<String, Object>();
		String hql = "FROM CarCash WHERE 1=1 ";
		hql += " AND docTypeId in (:idList)";
		...
		qp.setHql(hql);
		qp.setAlias(alias);
		QueryResult<CarCash> qr = carCashDao.listNoTotal(qp);
		return qr;
	}
