...
import com.zzb.zo.dao.CarDao;
import com.zzb.zo.dao.CarDao;
import com.zzb.zo.entity.Car;
import com.zzb.zo.entity.Car;
...
public class CarService extends BaseService<Car> {
	...
	public QueryResult<Car> list(HttpServletRequest request) {
		...
		qp.setArgs(args);
		qp.setHql(hql);
		QueryResult<Car> qr = carDao.list(qp);
		return qr;
	}
	...
	public Car save(Car doc, HttpServletRequest request) {
	...
	public Car dtl(int id) {
		Car doc = carDao.dtl(id);
		return doc;
	}
	...
	public QueryResult<Car> listByDocTypeId(int orgId) {
	...
	public QueryResult<Car> listAllByDocTypeIds(List<Integer> Ids) {
		QueryPara qp = new QueryPara();
		Map<String, Object> alias = new HashMap<String, Object>();
		String hql = "FROM Car WHERE 1=1";
		...
		qp.setAlias(alias);
		qp.setHql(hql);
		QueryResult<Car> qr = carDao.list4(qp);
		return qr;
	}



	public QueryResult<Car> listAllUser() {
		QueryPara qp = new QueryPara();
		//List<Object> args = new ArrayList<Object>();
		String hql = "FROM Car WHERE 1=1";
	...
	public QueryResult<Car> listByOrgIds(List<Integer> idList) {
		QueryPara qp = new QueryPara();
		Map<String, Object> alias = new HashMap<String, Object>();
		String hql = "FROM Car WHERE 1=1 ";
		hql += " AND docTypeId in (:idList)";
		...
		qp.setHql(hql);
		qp.setAlias(alias);
		QueryResult<Car> qr = carDao.listNoTotal(qp);
		return qr;
	}
