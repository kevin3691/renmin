...
import com.zzb.zo.dao.CarUseDao;
import com.zzb.zo.entity.CarUse;
...
public class CarUseService extends BaseService<CarUse> {
	 ...
	 * @return
	 */
	public QueryResult<CarUse> list(HttpServletRequest request) {
		...
		String carUsedNo = request.getParameter("carUsedNo") != null ? request
				.getParameter("carUsedNo") : "";
		...
		String hql = "FROM CarUse WHERE 1=1";
		if (isActive > 0) {
			hql += " AND isActive=?";
			args.add(isActive);
		}
		...
		qp.setArgs(args);
		qp.setHql(hql);
		QueryResult<CarUse> qr = carUseDao.list(qp);
		return qr;
	}
	


	public CarUse save(CarUse doc, HttpServletRequest request) {

		doc = carUseDao.save(doc);


		return doc;
	}

	public CarUse dtl(int id) {
		CarUse doc = carUseDao.dtl(id);
		return doc;
	}
	...
	public QueryResult<CarUse> listByDocTypeId(int orgId) {
	...
	public QueryResult<CarUse> listAllByDocTypeIds(List<Integer> Ids) {
		QueryPara qp = new QueryPara();
		Map<String, Object> alias = new HashMap<String, Object>();
		String hql = "FROM CarUse WHERE 1=1";
		...
		qp.setAlias(alias);
		qp.setHql(hql);
		QueryResult<CarUse> qr = carUseDao.list4(qp);
		return qr;
	}



	public QueryResult<CarUse> listAllUser() {
		QueryPara qp = new QueryPara();
		//List<Object> args = new ArrayList<Object>();
		String hql = "FROM CarUse WHERE 1=1";
	...
	public QueryResult<CarUse> listByOrgIds(List<Integer> idList) {
		QueryPara qp = new QueryPara();
		Map<String, Object> alias = new HashMap<String, Object>();
		String hql = "FROM CarUse WHERE 1=1 ";
		hql += " AND docTypeId in (:idList)";
		...
		qp.setHql(hql);
		qp.setAlias(alias);
		QueryResult<CarUse> qr = carUseDao.listNoTotal(qp);
		return qr;
	}
