...
import com.zzb.zo.service.SupTypeService;
...
public class SupTypeController extends BaseController {
	...
	public String edit(HttpServletRequest request, ModelMap model) {
		...
		SupType org = new SupType();
		if (id > 0) {
			org = supTypeService.dtl(id);
		}
		// 添加记录时，处理baseTree树属性
		if (parentId > 0) {
			SupType porg = supTypeService.dtl(parentId);
	...
	public QueryResult<SupType> list(HttpServletRequest request) {
		QueryResult<SupType> rslt = supTypeService.list(request);
		int count = 0;
		int total = 0;
//		for(SupType org : rslt.getRows())
//		{
//			count = supTypeService.getCount(org.getId());
//			total = supTypeService.getDocCount(org.getId());
//
//		}

		List<SupType> l = new ArrayList<>();
		l = rslt.getRows();
		SupType dt = new SupType();
		dt.setName("");
		dt.setId(0);
		l.add(0,dt);
		return rslt;
	...
	private void updateBaseTree(SupType org){
		QueryResult<SupType> rslt = supTypeService.listByParentId(org.getId());
		for(SupType o : rslt.getRows())
		{
	...
	public Map<String, Object> save(SupType org, HttpServletRequest request) {
		...
		if(org.getId() > 0)
		{
			...
			org = supTypeService.save(org, request);
			...
			int cnt = supTypeService.checkExist(org.getName());
			if (cnt == 0) {
