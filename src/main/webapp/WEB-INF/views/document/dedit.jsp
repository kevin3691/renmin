...
function onPrint() {
    ...
    function onSave (person) {
        ...
        //
	    // var para = $ ("#mainForm").serialize ();
	    $.post ('document/save_ins',{"docId":${o.id},"actorId":person.id,"actorName":person.name,"type":$("#type").select2("val")}, function (result, status) {
...
function ueSimpleUploadCallback(json){
	...
	function bindType(){
        $.post ("workflow/workflow/list", {
            ...
            if( "${o.type}" == 0){
                $ ("#type").select2 ("val",rows[0].id );
                $("#type" +rows[0].id).css("display","block")
			}
    ...
    function initIns(){
        $.post ("workflow/step/list", {
            ...
            html += '<li style="'+wcc+'">'
            html += '<div class="item current"><div class="step">'
            ...
            for (var i = 0; i < rows.length; i++){
                ...
                html += '<li style="'+wcc+'">'
				html += '<div class="item '+ cc +'"><div class="step">'
				html += '<i>'+index+'</i><label>'+rows[i].title+'</label>'
				html += '</div><span></span></div></li>'
            }

            html += '<li style="'+wcc+'">'
            html += '<div class="item last"><div class="step">'
...
<div id="form">
	...
	<div class="container-fluid">
        ...
        <br />
        见</span></td>
							<td colspan="7"  style="border-bottom:none;">${o.content}</td>
						</tr>
					</table>
        ...
        见</span></td>
					...
					<table width="627" height="44" border="0" bordercolor="#000000" cellspacing="0" class="dtable1">
						<tr>
                            ...
                            <div>
                            <button type="button" class="btn btn-info" onclick="onSel()">公文流转</button>
                            &nbsp;&nbsp;
