var mainPlatform = {
        ...
        for(var i = 0, len = menu.menu.length; i < len; i++){
           ...
           str += '<a href="javascript:;" class="pf-menu-title">'+
            ...
            for(var j = 0, jlen = m.children.length; j < jlen; j++){
            	...
            	if(child.isCurrent){
            		childStr += '<li class="active" text="'+ child.title +'" data-href="' + child.href + '"><a href="javascript:;">'+ child.title +'</a>' + this._renderThreeLevelMenu(child.children) + '</li>';
            		$('.easyui-tabs1[arrindex='+ index +']').tabs('add',{
						title: child.title,
            		...
            		childStr += '<li text="'+ child.title +'" data-href="' + child.href + '"><a href="javascript:;">'+ child.title +'</a>' + this._renderThreeLevelMenu(child.children) + '</li>';
            	}
            }
	...
	_renderThreeLevelMenu: function(list) {
		...
		for(;i<len;i++) {
			...
			html += '<li text="' + one.title +'" data-href="' + one.href + '"><a href="javascript:;">' + one.title + '</a></li>';
