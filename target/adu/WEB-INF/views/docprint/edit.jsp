...
<script type="text/javascript">
    ...
    function onSave () {
	    ...
	    $.post ('docprint/save', $ ("#mainForm").serialize (), function (result, status) {
			...
			parent.onQ()
			document.location.href = "docprint/edit/?id=" + result.entity.id
	    });
    }
    ...
    function onSubmit () {
        ...
        $.post ('docprint/submit', $ ("#mainForm").serialize (), function (result, status) {

            var id = result.entity.id;
            var total = $("#cash").jqGrid('getRowData')
            for(var i = 0 ;i < total.length;i++){
                var entity = new Object();
                entity.id = total[i].id
                entity.kemu = total[i].kemu
                entity.jine = total[i].jine
                entity.descr = total[i].descr
                entity.refNum = id
                entity.applyTo = "DOCPRINT"
                $.post('cash/details/save',{'cash':entity},function(result,status){
